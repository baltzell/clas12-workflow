...
chomp($date);
#$runcat = `ls -1 $indir/`;
$runcat = `readlink -f $indir/*`;
@runlist = split(/\n/,$runcat);

$yaml = "";
if($type eq "physics"){
     $yaml = "/home/rg-a/conductor/yaml/physics/physics_09_27_19.yaml";
}else{
...
for ($jobnumber = 0; $jobnumber< scalar @jobfiles; $jobnumber++){
    ...
    @fnamearr = split(/\//,$currentFileList[0]);
    #print($fnamearr[-1]);
    if(! -e "$outdir/skim_$fnamearr[-1]_4.hipo"){
    $njobs++;
    ...
    print OUT "COMMAND:          source /group/clas12/packages/setup.csh ; module load coatjava/6.3.1_4.3.11 ; module load ccdb/farm ; sh skim.sh $yaml $outdir                             \n";
